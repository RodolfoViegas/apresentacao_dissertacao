...
title-slide-attributes:
    data-background-image: imagens/ppgec_poli.png
...
# Introdução {background-color="#a0d5f6"}

![](imagens/forecast.png){.absolute top=240 left=63 height="120px"}
![](imagens/fable.png){ height="120px"}
![](imagens/tsibble.png){ height="120px"}
![](imagens/feast.png){ height="120px"}
![](imagens/fpp3.png){.absolute top=240 left=180 height="120px"}
![](imagens/smooth.png){.absolute top=240 left=295 height="120px"}


## Estudos de *Forecasting* <img src="imagens/forecast.png" height="120px"  style="vertical-align:middle;">
...
* Exemplos:
  ...
  2. Previsão de Gastos Governamentais;
  3. Previsão de Consumo de Energia.
:::



## Questões Importantes <img src="imagens/fpp3.png" height="120px"  style="vertical-align:middle;">
::: {.incremental}
* Limitações dos modelos individuais;

* Combinações de previsões como alternativa para ganho de acurácia;

* Há problemas em combinações? Por que algumas são melhores que outras?
  
  1. Má seleção de modelos individuais;
  2. Baixa diversidade;
  3. Complexidade.
* Fenômeno *forecast combination puzzle*.
:::

::: {.notes}
2- A literatura vem mostrando que a opção de combinar geralmente é a melhor forma de aumentar acuárcia. Mas qual tipo de combinação selecionar? todas são boas?

3 - Todos combinadores têm resultados bons? 
lichtendahl e Winkler
- má seleção, pooling;
- baixa diversidade, alta correlação dos resíduos;
- e complexidade, que pode aumenta o risco de acurácia

4- A pesar desse fenômeno curioso, só podemos, então aplicar a média simples? Não há outras formas como mostraram os autores citados. 

:::

## Objetivo Geral <img src="imagens/fable.png" height="120px"  style="vertical-align:middle;">

Analisar o desempenho de quatro operadores (média, mediana, operador poderado proposto e SVR-*stacking*) para combinações heterogêneas aplicadas à previsão de um passo à frente de séries temporais com sazonalidade mensal.

## Objetivos Específicos <img src="imagens/tsibble.png" height="120px"  style="vertical-align:middle;">

::: {.incremental}
1. Comparar operadores (média, mediana, média ponderada, *stacking*);
...
4. Avaliar operador ponderado proposto baseado em *feature importance* do modelo Extremely Randomized Trees.
:::


# Fundamentação Teórica {background-color="#a0d5f6"}

## Trabalhos Relacionados <img src="imagens/smooth.png" height="120px"  style="vertical-align:middle;">
::: {style="font-size: 80%;"}
...
3. O trabalho de @granger1984 emprega os coeficientes de regressão como pesos para média, ou seja, os $\beta$'s que **explicam** a regressão são usados.  
4. O modelo ET lida bem com **dados ruidosos** (@ghazwani2023computational) e tem a **variância menor** que outros modelos como Random Forest (@geurts2006extratrees).
:::
::: {.notes}
Alguns trabalhos importantes da áreas que usaram esses operadores

1 - Exemplo de operadores ponderados que venceram a média simples, Cada um de uma forma. é possível usa quase todas as coisas que falam algo, quantitativamente, em relação a modelos pode ser usada como pesos

2 - Esse sistema usou um sistema baseado em árvores

3 - o trabalho de Granger inspirou por usar um modelos como estimador de pesos. Semelhante à regressão modelos de Árvore também possuem um técnica que os explica: o *feature importance*. Árvore, porém, têm mais vantagens: **sem multicolinearidade** e **lida melhor que outras formas de distribuições**;

4 - agora por que usar o ET? As vantagem 
:::
...
::: {.notes}
1. De modo generalista, Zhou afirma que há um uma função que representa os dados, e pode ser impossível um modelo individual chegar nela, ma com combinações talvez;
2. Mackidadkis foca nos modelos individuais, suas dificuldade, como o pressuposto de constância nos padrões e distribuição, não é o caso de séries reais
...
Falar que o stackin usa SVR como meta-modelo com kernel RBF, modelo não linear
:::

# Metodologia {background-color="#a0d5f6"}
...
::: {.notes}
ao final, um vetor com valor média cada de importância para a variável. No caso do trabalho, a variável é a predição dos modelos individuais.
:::
...
### Características
::: goal
* É um *ensemble* de Árvore de Decisão semelhante ao Random Forest;
...
* Os dados são 30 séries de sazonalidade mensal da Competição M3;
...
# Resultados Individuais {background-color="#a0d5f6"}
...
# Resultado Geral {background-color="#a0d5f6"}
...
# Box Plots e Análise de Dispersão {background-color="#a0d5f6"}
...
::: {.notes}
 usando a analogia do alvo e dos dardos, modelos com baixa variância/desvio
padrão tendem a ser mais precisos em um determinado ponto. Reduzir a dispersão dos dados é diminuir o risco de acurácia (do inglês accuracy risk). definem risco de acurácia como o risco de sistemas gerarem previsões ruins ("the risk of a bad forecast"), os autores destacam que há uma relação entre variância e risco, em que ao diminuí-la, o risco dos modelos não performarem bem abranda-se.
:::
